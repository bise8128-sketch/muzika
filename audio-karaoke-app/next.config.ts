...
const nextConfig: NextConfig = {
  ...
  webpack: (config, { isServer }) => {
    ...
    // Configure WASM output paths - this is for WASM modules generated by webpack itself, not for pre-built ones
    config.output.webassemblyModuleFilename =
      isServer ? "../static/wasm/[modulehash].wasm" : "static/wasm/[modulehash].wasm";

    // Handle .wasm files, but exclude onnxruntime-web's wasm files as they are copied directly
    config.module.rules.push({
      test: /\.wasm$/,
      type: "webassembly/async",
      exclude: /onnxruntime-web/, // Exclude onnxruntime-web's WASM files
    });
