...
class PitchCorrectionProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this.config = {
      ...
      vocalAccuracy: 100, // Default to 100% (natural)
    };
  ...
  applyPitchCorrection(buffer, sampleRate) {
    ...
    const effectiveRatio =
      1 +
      (correctionRatio - 1) *
        this.currentCorrectionAmount *
        this.config.retuneSpeed;
  ...
  process(inputs, outputs, parameters) {
    if (this.config.adaptiveMode) {
      // Mapping Accuracy to Correction Amount:
      // > 90% accuracy: 0.2 (Natural sound, less assist)
      // < 70% accuracy: 0.95 (Full assist)
      // Interpolate in between
      const accuracy = this.config.vocalAccuracy;
      if (accuracy >= 90) {
        this.targetCorrectionAmount = 0.2;
      } else if (accuracy <= 70) {
        this.targetCorrectionAmount = 0.95;
      } else {
        // Linear interpolation: (90 - accuracy) / 20 * (0.95 - 0.2) + 0.2
        this.targetCorrectionAmount = 0.2 + ((90 - accuracy) / 20) * 0.75;
      }
    }

    // Smooth correction amount
    if (
      Math.abs(this.currentCorrectionAmount - this.targetCorrectionAmount) >
      0.001
    ) {
      this.currentCorrectionAmount +=
        (this.targetCorrectionAmount - this.currentCorrectionAmount) *
    ...
    for (let channel = 0; channel < numChannels; channel++) {
      ...
      if (this.config.enabled) {
        ...
        // Apply pitch correction using analysis buffer for pitch detection
        const corrected = this.applyPitchCorrectionWithAnalysisBuffer(
          inputData,
    ...
    this.metrics.averageCorrectionAmount =
      this.totalCorrections > 0
